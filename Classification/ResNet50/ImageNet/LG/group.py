...
##############################################################################################################
	def group(self):
		...
		if os.path.isfile("i_group_a50") and os.path.isfile("model_group_a50"):
		    self.model = torch.load("model_group_a50")
		    print("model_group resume:", self.model)
		    i_ = torch.load("i_group_a50")
		    print("i resume:", i_)

		    self.accuracys1 = torch.load("accuracys1_group_a50")
		    self.accuracys5 = torch.load("accuracys5_group_a50")
...
##############################################################################################################
	def finetune(self):
		...
		for i in [80, 70, 60]:
		    if os.path.isfile("model_group_a" + str(i)):
		        self.model = torch.load("model_group_a" + str(i))
		        print("model_group resume:", self.model)
...
#		if cp == 60:
#		    i_ = torch.load("i_group_ing")
#		    self.model = torch.load("model_group_ing")
#
#		    self.accuracys1 = torch.load("accuracys1_group_ing")
#		    self.accuracys5 = torch.load("accuracys5_group_ing")
#		    print("accuracys1_group resume:", self.accuracys1)
#		    print("accuracys5_group resume:", self.accuracys5)
#		    accuracy = max(self.accuracys1)
#		else:
#		    accuracy = 0
...
if __name__ == '__main__':
    ...
    model = torch.load("model_group_a80").module
    print("model_training:", model)
    torch.save(model.state_dict(), "model_group_L80.pth")
